...
if not openai_api_key:
    # This is the warning note we discussed.
    print("Warning: OPENAI_API_KEY not found. Voice features will be disabled.")
else:
    # Initialize the OpenAI client
    openai.api_key = openai_api_key
    print("OpenAI client initialized for Whisper.")
...
# [NEW] Pydantic models for the build feature
class BuildChatRequest(BaseModel):
    messages: List[ChatMessage]
    jd_text: str # We can pass this in for a targeted build

class BuildChatResponse(BaseModel):
    ai_response: str
    build_complete: bool # Flag to tell frontend the interview is over
...
async def generate_resume_endpoint(request: GenerateResumeRequest):
    ...
    try:
        ...
        # Use a built-in core font (Helvetica)
        # This removes the need for any .ttf files
        pdf.set_font("Helvetica", size=10)
        
        # We must encode the text to 'latin-1' (which these fonts use)
        # and replace any character it can't handle with a '?'
        # This prevents the app from crashing on special Unicode characters
        safe_text = final_resume_text.encode('latin-1', 'replace').decode('latin-1')
        
        pdf.multi_cell(0, 5, safe_text)
        
        # Output as bytes (as we did before)
...
async def transcribe_audio_endpoint(file: UploadFile = File(...)):
    ...
    try:
        ...
        audio_file.name = "recording.wav" # Whisper needs a file extension
...
async def start_build_chat_endpoint(request: BuildChatRequest): # Using BuildChatRequest
    """
    Starts the guided "build from scratch" interview.
    Can be targeted if a JD is provided.
    """
    is_general_request = (request.jd_text == DEFAULT_JD)
    first_prompt = ""
...
async def synthesize_and_analyze_endpoint(request: SynthesizeRequest):
    """
    Takes a build-chat history, synthesizes a resume,
    analyzes it, and returns the analysis.
    """
    try:
        ...
        **Your Output (Resume Text Only):**
        ...
        if not resume_text or len(resume_text) < 50: # Sanity check
             raise HTTPException(status_code=500, detail="Failed to synthesize resume from chat.")

        # Step 2: Analyze the new resume (this is our "loop-back")
        # We re-use our existing, powerful run_analysis function
